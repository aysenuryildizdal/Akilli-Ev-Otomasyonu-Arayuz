...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  try {
    ...
    $updateQuery = "UPDATE anasayfa SET 
                    ustbaslik = :ustbaslik, 
                    aciklama = :aciklama, 
                    linkmetın = :linkmetin, 
                    link = :link, 
    ...
    $statement->bindParam(':ustbaslik', $_POST['ustbaslik']);
    $statement->bindParam(':aciklama', $_POST['aciklama']);
    $statement->bindParam(':linkmetin', $_POST['linkmetın']);
    $statement->bindParam(':link', $_POST['link']);
    $statement->bindParam(':hakkimizda', $_POST['hakkimizda']);
    $statement->bindParam(':altlink', $_POST['altlink']);
    $statement->bindParam(':yorum', $_POST['yorum']);

    // İfadeyi çalıştır
    if ($statement->execute()) {
      echo "ID: 1 başarıyla güncellendi.";
      // Yönlendirme
      header("Location: adminanasay.php");
...
<body class="app sidebar-mini rtl">
  ...
  <main class="app-content">
    ...
    <div class="table table-hover table-bordered">
      ...
      $query = "SELECT * FROM anasayfa where ıd = 1";
      ...
      <form action="#" method="post" style="max-width: 800px; margin: 0 auto;">
        <div class="form-group row">
          <div class="col-md-6">
            <label for="ustbaslik">Başlık</label>
            <input type="text" id="ustbaslik" name="ustbaslik" required class="form-control"
              value="<?= $sonuc['ustbaslik'] ?>">
          </div>
          <div class="col-md-6">
            <label for="aciklama">Açıklama</label>
            <textarea id="aciklama" name="aciklama" required class="form-control"
              style="height: 150px;"><?= $sonuc['aciklama'] ?></textarea>
          </div>
        </div>
        <div class="form-group row">
          <div class="col-md-6">
            <label for="linkmetın">Link Metni</label>
            <input type="text" id="linkmetın" name="linkmetın" required class="form-control"
              value="<?= $sonuc['linkmetın'] ?>">
          </div>
        </div>
        <div class="form-group row">
          <div class="col-md-6">
            <label for="link">Link</label>
            <input type="text" id="link" name="link" required class="form-control" value="<?= $sonuc['link'] ?>">
          </div>
        </div>
        <div class="form-group row">
          <div class="col-md-6">
            <label for="hakkimizda">Hakkımızda</label>
            <input type="text" id="hakkimizda" name="hakkimizda" required class="form-control"
              value="<?= $sonuc['hakkimizda'] ?>">
          </div>
        </div>
        <div class="form-group row">
          <div class="col-md-6">
            <label for="altlink">Kullanıcı Yorumu</label>
            <input type="text" id="altlink" name="altlink" required class="form-control"
              value="<?= $sonuc['altlink'] ?>">
          </div>
        </div>
        <div class="form-group row">
          <div class="col-md-6">
            ...
            <input type="text" id="yorum" name="yorum" required class="form-control" value="<?= $sonuc['yorum'] ?>">
          </div>
        </div>
